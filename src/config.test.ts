...
test('session ID detection works correctly', () => {
  // Test the logic that checks if a session ID is present
  const checkSessionIdPresent = (sessionId: string): boolean => {
    return sessionId !== '';
  };

  expect(checkSessionIdPresent('session_123')).toBe(true);
  expect(checkSessionIdPresent('')).toBe(false);
});
...
test('default values work correctly', () => {
  const getValue = (envValue: string | undefined, defaultValue: string): string => {
