 ...
 * @returns Agent ID
 */
async function getOrCreateAgentForUser(
  userId: number,
  username?: string
): Promise<string> {
  ...
  try {
    console.log(`Creating new agent for user ${userId} (${username || "unknown"})`);
    ...
    const agentState = await client.agents.create({
      name: `user-${userId}-${username || "unknown"}`,
      description: `ADHD support agent for Telegram user ${userId}`,
...
Be supportive, understanding, and practical. Keep responses concise and actionable.`,
    ...
    console.log(`Created agent ${agentState.id} for user ${userId}`);
...
async function sendMessageToAgent(
  ...
  try {
    ...
    throw new Error(
      `Failed to get response from agent: ${error.message || "Unknown error"}`
    );
  }
}
...
bot.on("message", async (ctx: Context) => {
  ...
  try {
    ...
    await ctx.reply(errorMessage).catch((replyError) => {
